...
This specification addresses the first adverse effect by enabling a consumer of Conceptual Messages (the subscriber) to request via continuous stream of new or updated Conceptual Messages {{RFC8639}} via a subscription to an \<attestation\> Event Stream, which exists upon the producer of Conceptual Messages (the publisher). One prominent example is a Verifier's subscription to Evidence generated by an Attester. When subscribed, the producer of Conceptual Messages will continuously stream a requested set of Conceptual Messages to the subscriber.

The second adverse effect results from the nonce based challenge-response interaction model {{-rats-models}} realized in {{-rats-yang-tpm-charra}}. In that document an Attester must wait for a new nonce from a Verifier before it generates a new TPM Quote. To address delays resulting from such a wait, this specification enables freshness to be asserted asynchronously via the stream attestation interaction model {{-rats-models}}.

By removing these two adverse effects, it becomes possible, for example, for a RATS Verifier to continuously maintain an appraisal of an Attester's Evidence without relying on continuous polling.
