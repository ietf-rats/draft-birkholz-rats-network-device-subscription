...
The second adverse effect results from the nonce based challenge-response interaction model {{-rats-models}} realized in {{-rats-yang-tpm}}. In that document an Attester must wait for a new nonce from a Verifier before it generates a new TPM Quote. To address delays resulting from such a wait, this specification enables freshness to be asserted asynchronously via the stream attestation interaction model {{-rats-models}}.
